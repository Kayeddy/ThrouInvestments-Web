...
import { tagType, throu } from "../../assets";
...
const ExpandedCampaignView = ({ campaign, closeOverlay }) => {
  ...
  const handleNvaigation = () => {
    navigate(`/proyectos/${campaign.projectURI.title}`, { state: campaign });
  };
    ...
    >
      <motion.div className="flex flex-col justify-center h-full lg:w-[60%]">
        <motion.img
          ...
          alt={`images for: ${campaign.projectURI.title}`}
      ...
      <motion.div
        className="flex flex-col flex-1 flex-wrap justify-center items-center md:w-[40%] h-full mt-[25px] md:mt-0"
        variants={infoVariants}
      >
        <motion.h3
          className={`font-epilogue font-semibold text-[24px] text-white leading-26px text-center`}
        >
          {campaign.projectURI.title}
        </motion.h3>
        ...
        <motion.div className="flex items-center gap-[12px]">
          ...
          >
            Publicado por:{" "}
            <motion.span className="text-[#b2b3bd]">Throu</motion.span>
          </motion.p>
        </motion.div>

        <div className="flex flex-row flex-1  justify-center items-center gap-16 mt-12 md:w-[40%]">
          <CustomButton
            type="button"
            title="Volver"
